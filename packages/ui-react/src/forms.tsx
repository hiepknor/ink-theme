...
interface FieldCopyProps { description?: ReactNode; error?: ReactNode; errorLive?: FeedbackLive; id: string; }
function FieldCopy({ description, error, errorLive = 'off', id }: FieldCopyProps) {
  return <>{description && <div className="ink-ui-description" id={`${id}-description`}>{description}</div>}{error && <ErrorMessage id={`${id}-error`} live={errorLive}>{error}</ErrorMessage>}</>;
}

export interface TextAreaProps extends TextareaHTMLAttributes<HTMLTextAreaElement> {
  density?: InkDensity; description?: ReactNode; error?: ReactNode; errorLive?: FeedbackLive; label: ReactNode;
}
export const TextArea = forwardRef<HTMLTextAreaElement, TextAreaProps>(function TextArea(
  { 'aria-describedby': ariaDescribedBy, 'aria-invalid': ariaInvalid, className, density: densityOverride, description, error, errorLive = 'off', id: idProp, label, ...props }, ref,
) {
  const id = idProp ?? `ink-textarea-${useId()}`;
  const density = useInkDensity(densityOverride);
  return <div className="ink-ui-field" data-density={density}><label className="ink-ui-label" htmlFor={id}>{label}</label><textarea {...props} ref={ref} id={id} className={classes('ink-ui-input ink-ui-textarea', className)} aria-invalid={ariaInvalid ?? (error ? true : undefined)} aria-describedby={describedBy(ariaDescribedBy, description ? `${id}-description` : undefined, error ? `${id}-error` : undefined)} /><FieldCopy id={id} description={description} error={error} errorLive={errorLive} /></div>;
});
...
export interface SelectProps extends Omit<ButtonHTMLAttributes<HTMLButtonElement>, 'defaultValue' | 'onChange' | 'value'> {
  ...
  error?: ReactNode; errorLive?: FeedbackLive; label: ReactNode; name?: string; onOpenChange?: (open: boolean) => void;
