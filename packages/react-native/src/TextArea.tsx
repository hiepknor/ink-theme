...
export function TextArea({ density: densityOverride, description, editable = true, error, label, nativeID: nativeIdProp, numberOfLines = 4, ...props }: TextAreaProps) {
  ...
  return <View style={styles.field}><Text nativeID={labelId} style={styles.label}>{label}</Text><TextInput accessibilityLabel={label} accessibilityLabelledBy={labelId}
    accessibilityState={{ disabled: !editable }} aria-invalid={Boolean(error)} editable={editable} multiline nativeID={nativeID} numberOfLines={numberOfLines}
...
const styles = StyleSheet.create({
  field: { gap: nativeTokens.spacing.xs }, label: { color: nativeTokens.colors.foreground, fontSize: nativeTokens.fontSize.sm, fontWeight: nativeTokens.fontWeight.bold },
  input: { backgroundColor: nativeTokens.colors.surface, borderColor: nativeTokens.colors.borderStrong, borderWidth: nativeTokens.borderWidth.default, color: nativeTokens.colors.foreground, fontSize: nativeTokens.fontSize.md, padding: nativeTokens.spacing.md },
