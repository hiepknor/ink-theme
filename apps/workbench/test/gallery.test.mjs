...
test('workbench exercises the React vertical slice and every density', () => {
  assert.ok(legacyGallery.includes('id="react-preview"'));
...
test('gallery reviews every public screentone recipe', () => {
  for (const utility of [
    ...
    assert.ok(legacyGallery.includes(utility), `Gallery is missing ${utility}`);
  }
});

test('gallery keeps status meaning in visible text', () => {
  for (const label of ['Healthy', 'Starting', 'Degraded']) {
    assert.ok(legacyGallery.includes(label), `Gallery is missing status label ${label}`);
  }
});
...
test('gallery previews controls and interactive states', () => {
  for (const marker of ['<input', '<select', '<textarea', 'disabled', 'readOnly', 'required', 'placeholder', 'aria-invalid="true"', 'focus-visible']) {
    assert.ok(legacyGallery.includes(marker), `Gallery is missing control state ${marker}`);
  }
});
...
test('workbench uses one React rendering architecture', () => {
  assert.ok(gallery.includes('id="root"'));
  assert.ok(gallery.includes('src="./main.tsx"'));
  assert.ok(!gallery.includes('react-preview.tsx'));
  assert.ok(!gallery.includes('gallery.js'));
  assert.ok(reactPreview.includes("document.querySelector('#root')"));
});
